...
class GlobalDataHandler extends DefaultHandler {
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		if (qName.equals(Serializer.GLOBAL_DATA_TAG)) {
			...
			Mode mode = new Mode(id, vdcs[1], vdcs[0], false); //TODO cheque book support
			...
			this.data.add(new PeriodicalTransaction(id, p.description, p.amount, p.account, p.mode, p.category, lst, nextDate, enabled, anticipation, stepper));
		} else if (qName.equals(Serializer.DATE_STEPPER_TAG)) {
		} else {
			System.err.println ("Unknown tag "+qName);
		}
