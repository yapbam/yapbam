 ...
 * <P>
 *
 * @author Darryl Burke (https://tips4java.wordpress.com/) modified by JM Astesana
 */
public class MenuScroller {
   ...
   * @param topFixedCount the number of items to fix at the top.  May be 0
   * @param bottomFixedCount the number of items to fix at the bottom.  May be 0
   * @param addSeparators true to add separators before and after the scrolling region (this is the default).
   * @throws IllegalArgumentException if scrollCount or interval is 0 or
   * negative or if topFixedCount or bottomFixedCount is negative
   */
  public MenuScroller(JMenu menu, int scrollCount, int interval, int topFixedCount, int bottomFixedCount, boolean addSeparators) {
    this(menu.getPopupMenu(), scrollCount, interval, topFixedCount, bottomFixedCount, addSeparators);
  }
  ...
  public MenuScroller(JPopupMenu menu, int scrollCount, int interval, int topFixedCount, int bottomFixedCount) {
  	this (menu, scrollCount, interval, topFixedCount, bottomFixedCount, true);
  }
  /**
   * Constructs a <code>MenuScroller</code> that scrolls a popup menu with the
   * specified number of items to display in the scrolling region, the
  ...
  private void refreshMenu() {
    if (menuItems != null && menuItems.length > 0) {
      ...
      if (addSeparators && topFixedCount > 0) {
        menu.addSeparator();
      }
      ...
      if (addSeparators && bottomFixedCount > 0) {
        menu.addSeparator();
      }
