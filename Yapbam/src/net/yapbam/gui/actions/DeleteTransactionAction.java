...
public class DeleteTransactionAction extends AbstractAction {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (editingOptions.isAlertOnModifyChecked() && (transaction.getStatement()!=null)) {
			AlertDialog alert = new AlertDialog(AbstractDialog.getOwnerWindow((Component)e.getSource()), LocalizationData.get("DeleteCheckedTransactionAlert.title"), LocalizationData.get("DeleteCheckedTransactionAlert.message")); //$NON-NLS-1$ //$NON-NLS-2$
			...
			AlertDialog alert = new AlertDialog(AbstractDialog.getOwnerWindow((Component)e.getSource()), LocalizationData.get("DeleteTransactionAlert.title"), LocalizationData.get("DeleteTransactionAlert.message")); //$NON-NLS-1$ //$NON-NLS-2$
			alert.setVisible(true);
			if (alert.getResult()==null) return;
			if (alert.getResult()) {
